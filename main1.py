"""
本代码实现的是软件工程试验一内容，实验具体需求详见实验一ppt
"""
...
class DirectedGraphProcessor:
    """
    该类包含本实验所需全部功能的实现，包括有向图生成及展示，查找桥接词，生成新文本，查找最短路，随机游走
    """
    def __init__(self):
        self.graph = {}
    ...
    def create_directed_graph(self, file_name):
        ...
        for u_node in self.graph:
            self.graph[u_node]=Counter(self.graph[u_node])
            for v_node in self.graph[u_node]:
                self.direct_graph.add_edge(u_node, v_node, weight=self.graph[u_node][v_node])
    ...
    def show_directed_graph(self):
        ...
        展示有向图
        """
        pos = nx.spring_layout(self.direct_graph)
        labels = nx.get_edge_attributes(self.direct_graph, 'weight')
        nx.draw_networkx(self.direct_graph, pos, node_size=800)
        nx.draw_networkx_edge_labels(self.direct_graph, pos, edge_labels=labels)
        plt.show()
    ...
    def query_bridge_words(self, word_1, word_2):
        ...
        """
        # 如果word1或word2不在图中
        if word_1 not in self.graph or word_2 not in self.direct_graph.nodes:
            return 'NOT_IN'
        # 如果word1和word2都在图中
        if (word_1,word_2) in self.bridge_word:
    ...
    def calc_shortest_path(self, word_1, word_2):
        ...
        elif word_1 != '' and word_1 not in self.direct_graph.nodes and (word_2 in self.direct_graph.nodes or word_2 == ''):
            messagebox.showerror("Error", f"No '{word_1}' in the graph.")

        # word2不为空且不在图中，word1为空或者在图中
        elif word_2 != '' and word_2 not in self.direct_graph.nodes and (word_1 in self.direct_graph.nodes or word_1 == ''):
            messagebox.showerror("Error", f"No '{word_2}' in the graph.")

        # word1和word2都不为空且都不在图中
        elif word_1 not in self.direct_graph.nodes and word_2 not in self.direct_graph.nodes:
            messagebox.showerror("Error", f"No '{word_1}' and '{word_2}' in the graph.")

        # 可选功能：只输入了一个单词
        elif (word_1 == '' and word_2 in self.direct_graph.nodes) or (word_2 == '' and word_1 in self.direct_graph.nodes):
            # 调用draw_graph绘制图
            word_input = word_1 if word_1 != '' else word_2
            for word in self.direct_graph.nodes:
                try:
                    # 计算最短路径
                    min_path = nx.dijkstra_path(self.direct_graph, source=word_input, target=word)
                    # 计算最短路径长度
                    min_path_length = nx.dijkstra_path_length(self.direct_graph, source=word_input, target=word)
        ...
        # word1和word2都在图中
        elif word_1 in self.direct_graph.nodes and word_2 in self.direct_graph.nodes:
            # 调用draw_graph绘制图
            try:
                # 计算最短路径
                min_path = nx.dijkstra_path(self.direct_graph, source=word_1, target=word_2)
                # 计算最短路径长度
                min_path_length = nx.dijkstra_path_length(self.direct_graph, source=word_1, target=word_2)
                ...
                nx.draw_networkx_edges(self.direct_graph, pos, edgelist=edge_list, edge_color='m', width=4)
                plt.show()
    ...
    def draw_graph(self):
        ...
        for key, value in self.graph.items():
            ...
            node1 = key[0]
            node2 = key[1]
            direct_graph.add_edge(node1, node2, weight=value)
        return direct_graph
    ...
    def random_walk(self):
        ...
        random_walk_edges = []
        # 随机选择起点
        random_word = random.choice(list(self.direct_graph.nodes))
        random_walk_list.append(random_word)
        while 1:
            if random_word not in self.graph:
                break
